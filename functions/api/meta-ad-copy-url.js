import { jsonResponse, readJson, getMetaToken, safeJson } from "../_utils.js";

const API_BASE = "https://graph.facebook.com/v24.0";

export async function onRequest({ request, env }) {
  const token = getMetaToken(env);
  if (!token) {
    return jsonResponse(500, { error: "META_ACCESS_TOKEN nao configurado" });
  }

  if (request.method !== "POST") {
    return jsonResponse(405, { error: "Method not allowed" });
  }

  const body = await readJson(request);
  const { ad_id, adset_id, link_url, url_tags, name, status_option } = body || {};
  if (!ad_id || !link_url) {
    return jsonResponse(400, {
      error: "Parametros obrigatorios: ad_id, link_url",
    });
  }

  try {
    const payload = new URLSearchParams();
    if (adset_id) payload.set("adset_id", adset_id);
    payload.set("status_option", status_option || "PAUSED");
    if (name) {
      payload.set("rename_options", JSON.stringify({ rename_strategy: "NO_RENAME" }));
      payload.set("name", name);
    }
    payload.set(
      "creative",
      JSON.stringify({
        link_url: link_url,
        url_tags: url_tags || "",
      })
    );
    payload.set("access_token", token);

    const response = await fetch(
      `${API_BASE}/${encodeURIComponent(ad_id)}/copies`,
      {
        method: "POST",
        body: payload,
      }
    );
    const data = await safeJson(response);
    if (!response.ok) {
      return jsonResponse(response.status, { error: "Erro Meta", details: data });
    }
    return jsonResponse(200, { code: "success", data });
  } catch (error) {
    return jsonResponse(500, {
      error: "Erro ao copiar anuncio",
      details: error.message,
    });
  }
}
